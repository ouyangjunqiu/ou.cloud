...
class File {
	...
	public static function copyToDir( $file, $copyToPath ) {
		if ( self::fileExists( $file ) ) {
			...
			if ( LOCAL ) {
				...
				$state = Cloud::engine()->io()->file()->moveFile( $file, $copyToPath . $name );
			}
			return $state;
		}
	}
	 ...
	 * @param $data
	 */
	public static function exportExcel($filename, $data = array()){
		$excel = new PHPExcel();
		foreach($data as $i=>$row){
			foreach($row as $j=>$value){
				$excel->getActiveSheet()->setCellValueByColumnAndRow($j, $i+1,$value);
			}
		}
	 ...
	 */
	public static function downloadCsvData($filename, $csv_data = array())
	{
		$csv_string = null;
		$csv_row = array();
		...
		$csv_string = implode(PHP_EOL, $csv_row).PHP_EOL;
//
		$csv_string = "\xFF\xFE" .mb_convert_encoding($csv_string, 'UCS-2LE', 'UTF-8');
		self::exportCsv($filename, $csv_string);
	}
	...
	public static function downloadCrxFile($filename,$output_name){
		if(file_exists($filename)){
			$content = file_get_contents($filename);//将文件读入字符串
