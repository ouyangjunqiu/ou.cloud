...
class String{
	...
	public static function isUrl( $str ){
		$rule = '(((f|ht){1}t(p|ps)://)[-a-zA-Z0-9@:%_\+.~#?&//=\!]+)';
	...
	public static function authCode( $string, $operation = 'DECODE', $key = '', $expiry = 0 ) {
		$ckeyLength = 4;
		$key = md5( $key != '' ? $key : Cloud::app()->setting->get( 'authkey' )  );
	...
	public static function getSubIp( $ip = '' ) {
		if ( empty( $ip ) ) {
			$ip = $clientIp = Cloud::app()->setting->get( 'clientip' );
		}
		$reg = '/(\d+\.)(\d+\.)(\d+)\.(\d+)/';
	 ...
	 *  $data = Cloud:app()->setting->get('cache/positioncategory');
	...
	public static function parseForApi( $html ) {
		...
		$html = str_replace( array( '[SITE_URL]', '&nbsp;' ), array( Cloud::app()->setting->get( 'siteurl' ), ' ' ), $html );
		//@提到某人处理
	...
	private static function parseTheme( $data ) {
		// 如果话题被锁定，则不带链接
		$lock = Cloud::app()->db->createCommand()
				->select( 'lock' )
				->from( '{{feed_topic}}' )
				->where( sprintf( "topicname = '%s'", $data[1] ) )
				->queryScalar();
		if ( !$lock ) {
			return "<a class='wb-source' href=" . Cloud::app()->urlManager->createUrl( 'weibo/topic/detail', array( 'k' => urlencode( $data[1] ) ) ) . ">" . $data[0] . "</a>";
		} else {
			return $data[0];
		}
	}
	...
	public static function replaceUrl( $content ) {
		...
		$content = str_replace( '[SITE_URL]', Cloud::app()->setting->get( 'siteurl' ), $content );
		$content = preg_replace_callback( '/((?:https?|mailto|ftp):\/\/([^\x{2e80}-\x{9fff}\s<\'\"“”‘’，。}]*)?)/u', 'self::parseUrl', $content );
		return $content;
	}

	public static function ireplaceUrl( $content , $replacement ){
		$rule = '(((f|ht){1}t(p|ps)://)[-a-zA-Z0-9@:%_\+.~#?&//=\!]+)';
		return preg_replace($rule, $replacement, $content);
	}
	...
	public static function isEmpty($str){
		...
		if(trim($str) == "" || strtolower($str) == "null" || strtolower($str) == "undefined"){
			return $str;
		}
		return $str;
	}
