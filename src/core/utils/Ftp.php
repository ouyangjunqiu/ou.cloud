...
class Ftp {
    ...
    public function __construct( $config = array() ) {
        $ftp = Cloud::app()->setting->get( 'setting/ftp' );
        $this->setError( 0 );
        ...
        if ( empty( $this->_config['on'] ) || empty( $this->_config['host'] ) ) {
            ...
            if ( $this->_func == 'ftpConnect' && !function_exists( 'ftpConnect' ) ) {
                ...
                $this->_config['host'] = $this->clear( $this->_config['host'] );
                $autoKey = md5( Cloud::app()->setting->get( 'config/security/authkey' ) );
    ...
    public function upload( $source, $target ) {
        ...
        if ( !$this->ftpChdir( $dirName ) ) {
            if ( $this->ftpMkdir( $dirName ) ) {
                ...
                $attachDir = Cloud::app()->setting->get( 'setting/attachdir' );
    ...
    public function connect() {
        if ( !$this->_enabled || empty( $this->_config ) ) {
            ...
            return $this->ftpConnect(
                $this->_config['host'], $this->_config['username'], $this->_config['password'], $this->_config['attachdir'], $this->_config['port'], $this->_config['timeout'], $this->_config['ssl'], $this->_config['pasv']
            );
        }
    }
     ...
     * @param string|int $ftpPort
     * @param integer $timeout
     * @param integer $ftpssl
     * @param string|int $ftpPasv
     * @return integer
     */
